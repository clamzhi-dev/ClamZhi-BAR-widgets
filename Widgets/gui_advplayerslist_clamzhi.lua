...
function widget:GetInfo()
	return {
		name = "ClamZhi AdvPlayersList",
		desc = "AdvPlayersList fork: Ctrl+drag reorder, sort by recommended start positions, position-name display",
		author = "Marmoth, Floris (original AdvPlayersList); ClamZhi (fork)",
		date = "2008",
		version = 46,
...
-- [CUSTOM] Ctrl+drag manual player/team ordering + "autosort by battlefield position".
-- Stored on WG (not a file-scope local) and helpers are globals: this file is already
-- at Lua's 200-locals-per-chunk limit. Each function that needs it does
-- `local reorder = WG.aplc` locally.
--   reorder.order      : teamID -> numeric position (teams not moved keep engine order).
--                        In-memory only, reset each widget load (Initialize).
--   reorder.drag       : active Ctrl-drag { teamID, allyTeam, startX, startY, moved, objID, canMute, muteKey }
...
--------------------------------------------------------------------------------
-- [CUSTOM] helpers for Ctrl+drag manual ordering + autosort-by-position.
-- All globals on purpose: this file is at Lua's 200-local-per-chunk limit.
--------------------------------------------------------------------------------

-- ordered array of teamIDs for the ally team, and map teamID -> effective pos.
-- effective pos = reorder.order[teamID] if the user moved it, else its engine index.
function ReorderOrdered(baseList)
	local reorder = WG.aplc
	local eff = {}
	local list = {}
	for idx = 1, #baseList do
		local t = baseList[idx]
		eff[t] = reorder.order[t] or (idx - 1)
		list[idx] = t
	end
...
function ReorderBaseList(allyTeamID)
	return teamRanking[allyTeamID] or sp.GetTeamList(allyTeamID)
end

-- Called from SortTeams: apply reorder.order on top of the engine/leaderboard list.
-- Never mutates the passed table (teamRanking tables are persistent).
function ApplyManualOrder(teamsList)
	local reorder = WG.aplc
	for idx = 1, #teamsList do
		if reorder.order[teamsList[idx]] ~= nil then
			return (ReorderOrdered(teamsList))
		end
	end
	return teamsList
end

-- Move draggedID to sit just before / after targetID within their ally team.
-- Returns true if the order actually changed.
function ReorderApply(allyTeamID, draggedID, targetID, placeAfter)
	local reorder = WG.aplc
	if draggedID == targetID then
		return false
	end
	local list, eff = ReorderOrdered(ReorderBaseList(allyTeamID))
	local di, ti
	for idx = 1, #list do
		if list[idx] == draggedID then di = idx end
		if list[idx] == targetID then ti = idx end
	end
	if not di or not ti then
		return false
	end
	if placeAfter and di == ti + 1 then return false end
	if (not placeAfter) and di == ti - 1 then return false end

	local lo, hi
	if placeAfter then
		lo = eff[targetID]
		hi = list[ti + 1] and eff[list[ti + 1]] or (lo + 1)
	else
		hi = eff[targetID]
		lo = list[ti - 1] and eff[list[ti - 1]] or (hi - 1)
	end
	reorder.order[draggedID] = (lo + hi) * 0.5
	return true
end

-- Lock in a clean integer ordering for an ally team (called on drag release)
function ReorderNormalize(allyTeamID)
	local reorder = WG.aplc
	local list = ReorderOrdered(ReorderBaseList(allyTeamID))
	for idx = 1, #list do
		reorder.order[list[idx]] = idx - 1
	end
end

-- Find the reorderable team row under the cursor.
-- Returns teamID, allyTeamID, rowScreenY  (or nil)
function ReorderRowUnder(x, y)
	for _, i in ipairs(drawList) do
		if i > -1 then
			local p = player[i]
			if p and p.name and p.posY and p.team ~= nil and p.spec ~= true then
				local rowY = widgetPosY + widgetHeight - p.posY
				if IsOnRect(x, y, widgetPosX, rowY, widgetPosX + widgetWidth, rowY + (playerOffset * playerScale)) then
					return p.team, p.allyteam, rowY
				end
			end
		end
	end
...
-- Order one ally team's rows to match the battlefield layout, reading each team's
-- chosen start position. The list is spread along whichever world axis (X = west->
-- east, Z = north->south) the team varies most on, so it reads top-to-bottom the
-- same way you read the minimap. Writes reorder.order. Returns true if it changed.
function ReorderByStartPos(allyTeamID)
	local reorder = WG.aplc
	local teams = sp.GetTeamList(allyTeamID)
	if not teams then
		return false
	...
	for _, t in ipairs(teams) do
		local px, _, pz = Spring.GetTeamStartPosition(t)
		if px and px > -99 then -- (-100,-100,-100) => no start position chosen yet
			pts[t] = { x = px, z = pz }
			ids[#ids + 1] = t
		end
	end
	...
	local n, sx, sz, sxx, szz = #ids, 0, 0, 0, 0
	for _, t in ipairs(ids) do
		local p = pts[t]
		sx, sz = sx + p.x, sz + p.z
		sxx, szz = sxx + p.x * p.x, szz + p.z * p.z
	end
	local varX = sxx / n - (sx / n) ^ 2
	local varZ = szz / n - (sz / n) ^ 2
	local axis = (varX >= varZ) and "x" or "z"
	tableSort(ids, function(a, b)
		if pts[a][axis] ~= pts[b][axis] then
			return pts[a][axis] < pts[b][axis]
		end
		return a < b
	end)
	for idx, t in ipairs(ids) do
		reorder.order[t] = idx - 1
	end
	return true
end

-- Load the map's "expert recommended start positions" (same source as the
-- Start Position Suggestions widget: the mapmetadata_startpos modoption).
-- Returns an array of sides; each side is an ordered array of
-- { spawnPoint={x,z}, role, name }, matching the current match's team count /
-- players-per-team. Returns false if unavailable. Cached on WG.aplc.suggest.
function LoadStartSuggestions()
	local raw = Spring.GetModOptions().mapmetadata_startpos
	if not raw or raw == "" then
		return false
	end
	local ok, sides = pcall(function()
		local base64 = VFS.Include("common/luaUtilities/base64.lua")
		local parsed = Json.decode(VFS.ZlibDecompress(base64.Decode(raw)))
		local gaiaAT = select(6, Spring.GetTeamInfo(Spring.GetGaiaTeamID(), false))
		local atCount, perTeam = 0, 0
		for _, at in ipairs(Spring.GetAllyTeamList()) do
			if at ~= gaiaAT then
				atCount = atCount + 1
				perTeam = math.max(perTeam, #(Spring.GetTeamList(at) or {}))
			end
		end
		local cfgSides
		for _, tc in ipairs(parsed.team) do
			if tc.playersPerTeam == perTeam and tc.teamCount == atCount then
				cfgSides = tc.sides
				break
			end
		end
		if not cfgSides then
			return nil
		end
		local out = {}
		for i, side in ipairs(cfgSides) do
			out[i] = {}
			for _, st in ipairs(side.starts) do
				local p = parsed.positions[st.spawnPoint]
				if p then
					out[i][#out[i] + 1] =
						{ spawnPoint = { x = p.x, z = p.y }, role = st.role, name = tostring(st.spawnPoint) }
				end
			end
		end
		return out
	end)
	return (ok and sides) or false
end

-- Case-insensitive sequence equality for role lists.
function SeqEqCI(a, b)
	if #a ~= #b then
		return false
	end
	for i = 1, #a do
...
function StartposProfile(roles)
	...
	--   1 Sea  2 Tech  3 Air  4 Front  5 Front  6 Front  7 Front/Tech  8 Sea
	if SeqEqCI(roles, { "Sea", "Tech", "Air", "Front", "Front", "Front", "Front/Tech", "Sea" }) then
		return {
			rank = { [5] = 1, [6] = 2, [4] = 3, [7] = 4, [8] = 5, [1] = 6, [2] = 7, [3] = 8 },
			labels = {
				[5] = "Front 1",
				[6] = "Front 2",
				[4] = "Pond",
...
-- Core recommendation matching for one ally team.
-- Returns: best (ordered slot array), rank (teamID -> {slot=nativeIdx, dist}), profile  -- or nil
function SuggestedSlots(allyTeamID)
	local reorder = WG.aplc
	if reorder.suggest == nil then
		reorder.suggest = LoadStartSuggestions()
	end
	local data = reorder.suggest
	if not data then
		return nil
	end
	local teams = sp.GetTeamList(allyTeamID)
	...
	-- reference point: mean of placed starts, else start-box center
	local rx, rz, cnt = 0, 0, 0
	for _, t in ipairs(teams) do
		local x, _, z = Spring.GetTeamStartPosition(t)
		if x and x > -99 then
			rx, rz, cnt = rx + x, rz + z, cnt + 1
		end
	end
	if cnt > 0 then
		rx, rz = rx / cnt, rz / cnt
	else
		local x0, z0, x1, z1 = Spring.GetAllyTeamStartBox(allyTeamID)
		if not x0 then
			return nil
		end
		rx, rz = (x0 + x1) * 0.5, (z0 + z1) * 0.5
	end

	-- pick the recommended side whose spawn-point centroid is closest
	local best, bestD
	for _, side in ipairs(data) do
		local sx, sz, sc = 0, 0, 0
		for _, pos in ipairs(side) do
			sx, sz, sc = sx + pos.spawnPoint.x, sz + pos.spawnPoint.z, sc + 1
		end
		if sc > 0 then
			local d = (sx / sc - rx) ^ 2 + (sz / sc - rz) ^ 2
	...
	local profile = StartposProfile(roles)
	...
	for _, t in ipairs(teams) do
		...
		if x and x > -99 then
			local bi, bd
			for j = 1, #best do
				local s2 = best[j].spawnPoint
				local d = (s2.x - x) ^ 2 + (s2.z - z) ^ 2
				if not bd or d < bd then
					bd, bi = d, j
				end
			end
			rank[t] = { slot = bi, dist = bd }
		end
	end
...
function RefreshSlotLabels()
	...
	for _, at in ipairs(ats) do
		...
		if best then
			ApplySlotLabels(best, rank, profile)
		end
	end
end

-- Order an ally team's rows to match the map's recommended start positions.
-- Each player is matched to the nearest recommended slot; the list follows the
-- profile's slot order if there is one, otherwise the recommendation's own order.
-- Also refreshes WG.aplc.slotLabel for these teams. Writes reorder.order.
function ReorderBySuggestedPos(allyTeamID)
	local reorder = WG.aplc
	local best, rank, profile = SuggestedSlots(allyTeamID)
	if not best then
		return false
	end
	ApplySlotLabels(best, rank, profile)
	...
	for _, t in ipairs(teams) do
		if rank[t] then
			...
			unplaced[#unplaced + 1] = t
		end
	end
	if #placed < 2 then
		return false
	end
	local function desiredRank(t)
		return (profile and profile.rank[rank[t].slot]) or rank[t].slot
	end
	tableSort(placed, function(a, b)
		local ra, rb = desiredRank(a), desiredRank(b)
		if ra ~= rb then
			return ra < rb
		end
		if rank[a].dist ~= rank[b].dist then
			return rank[a].dist < rank[b].dist
		end
		return a < b
	end)
...
-- Print the loaded recommendation slots + which player matched each, for tuning
-- a StartposProfile. Bound as action "advplayerlist_startpos_dump".
function DumpStartSuggestions()
	local best, rank, profile = SuggestedSlots(mySpecStatus and (sp.GetAllyTeamList()[1]) or myAllyTeamID)
	if not best then
		Spring.Echo("[ClamZhi AdvPlayersList] no recommendation data for this map/teamsize")
		return
	end
	local matchedBy = {}
	for t, r in pairs(rank) do
		local leaderID = select(2, Spring.GetTeamInfo(t, false))
		local nm = (leaderID and leaderID >= 0) and select(1, Spring.GetPlayerInfo(leaderID, false)) or nil
		nm = nm or (player[t + specOffset] and player[t + specOffset].name) or ("team " .. t)
		matchedBy[r.slot] = (matchedBy[r.slot] and (matchedBy[r.slot] .. ",") or "") .. nm
	end
	Spring.Echo("[ClamZhi AdvPlayersList] recommendation slots (profile "
		.. (profile and "MATCHED" or "none") .. "):")
	for j = 1, #best do
		Spring.Echo(string.format(
			"  slot %d  role=%s  id=%s  pos=(%d,%d)  -> %s",
			j, tostring(best[j].role), tostring(best[j].name),
			best[j].spawnPoint.x, best[j].spawnPoint.z, matchedBy[j] or "-"))
	end
end

-- Sort your own ally team to match the map's recommended start positions (or every
-- ally team, when spectating); falls back to the spread-axis coordinate sort when
-- the map has no recommendation data. Triggered by the corner button and the
-- "advplayerlist_autosort_by_position" action.
function AutosortByPosition()
	local changed, usedSuggest = false, false
	if mySpecStatus then
		for _, at in ipairs(sp.GetAllyTeamList()) do
			if ReorderBySuggestedPos(at) then
				changed, usedSuggest = true, true
			elseif ReorderByStartPos(at) then
				changed = true
			end
		end
	elseif ReorderBySuggestedPos(myAllyTeamID) then
		changed, usedSuggest = true, true
	elseif ReorderByStartPos(myAllyTeamID) then
		changed = true
	end
	if changed then
		...
		SortList()
		CreateLists()
		Spring.Echo("[ClamZhi AdvPlayersList] sorted by "
			.. (usedSuggest and "recommended start positions" or "start-position spread"))
	end
	return changed
end
...
function SortTeams(allyTeamID, vOffset)
	...
	teamsList = ApplyManualOrder(teamsList) -- [CUSTOM: reorder]
...
function DrawAutosortButton()
	WG.aplc.buttonRect = nil
	if not RectRound then
		return
	end
	local sortableCount = mySpecStatus and #sp.GetTeamList() or #sp.GetTeamList(myAllyTeamID)
	if sortableCount <= 1 then
		return
	end
	...
	local hover = mx >= bx0 and mx <= bx1 and my >= by0 and my <= by1
	-- keep it visible pre-game, while hovering the list, or while hovering the button itself
	if not (spGetGameFrame() <= 0 or hoverPlayerlist or hover) then
		return
	end
	WG.aplc.buttonRect = { bx0, by0, bx1, by1 }
	local a = hover and 0.95 or 0.6
	...
	local u = (bx1 - bx0) / 15 -- glyph unit, scales with the button
	local gx = bx0 + 3 * u
	local gy = by0 + 4 * u
	gl.Rect(gx, gy + 6 * u, gx + 9 * u, gy + 7 * u)
	gl.Rect(gx, gy + 3 * u, gx + 6 * u, gy + 4 * u)
	gl.Rect(gx, gy, gx + 3 * u, gy + u)
	gl_Color(1, 1, 1, 1)
	gl.Blending(GL.SRC_ALPHA, GL.ONE_MINUS_SRC_ALPHA)
end
...
function widget:MousePress(x, y, button)
	...
	if button == 1 then
		...
		-- [CUSTOM: autosort] corner button: sort the list by battlefield position
		local btn = WG.aplc.buttonRect
		if btn and x >= btn[1] and x <= btn[3] and y >= btn[2] and y <= btn[4] then
			AutosortByPosition()
			return true
		...
		-- [CUSTOM: reorder] Ctrl-press on a player/team row begins a drag-to-reorder.
		-- A Ctrl-click without dragging still falls back to the normal mute toggle
		-- (handled in MouseRelease).
		if ctrl then
			for _, i in ipairs(drawList) do
				if i > -1 then
					local p = player[i]
					if p and p.name and p.posY and p.team ~= nil and p.spec ~= true then
						local rowY = widgetPosY + widgetHeight - p.posY
						if
							IsOnRect(x, y, widgetPosX, rowY, widgetPosX + widgetWidth, rowY + (playerOffset * playerScale))
							and (mySpecStatus or p.allyteam == myAllyTeamID)
						then
							WG.aplc.drag = {
								teamID = p.team,
								allyTeam = p.allyteam,
								...
								moved = false,
								objID = i,
								canMute = (i < specOffset) and not p.ai,
								muteKey = p.accountID or p.name,
							}
							return true
						end
					end
...
function widget:MouseMove(x, y, dx, dy, button)
	-- [CUSTOM: reorder] live drag-to-reorder while Ctrl-dragging a player row
	local drag = WG.aplc.drag
	if drag then
		if
			not drag.moved
			and (math.abs(x - drag.startX) + math.abs(y - drag.startY)) >= WG.aplc.threshold
		then
			drag.moved = true
		end
		if drag.moved then
			local targetTeam, targetAlly, targetRowY = ReorderRowUnder(x, y)
			if targetTeam and targetAlly == drag.allyTeam and targetTeam ~= drag.teamID then
				-- screen Y grows upward; cursor in the lower half of the target row => drop below it
				local placeAfter = y < (targetRowY + (playerOffset * playerScale) * 0.5)
				if ReorderApply(drag.allyTeam, drag.teamID, targetTeam, placeAfter) then
					SortList()
					CreateLists()
				end
			end
...
function widget:MouseRelease(x, y, button)
	if button == 1 then
		-- [CUSTOM: reorder] finish a Ctrl-drag reorder, or fall back to Ctrl+click mute
		if WG.aplc.drag then
			local drag = WG.aplc.drag
			WG.aplc.drag = nil
			if drag.moved then
				ReorderNormalize(drag.allyTeam)
				SortList()
				CreateLists()
			elseif drag.canMute and drag.muteKey then
				sp.SendCommands("toggleignore " .. drag.muteKey)
			end
			return true
		end
